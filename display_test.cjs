...
function setup(dpr=2) {
 ...
 const context=vm.createContext({console,PACKET_TYPES:{configure_display:"configure-display"},window:browser,document:{querySelector:()=>shadow},jQuery:()=>({scrollLeft:()=>0,scrollTop:()=>0})});
 ...
 Object.assign(client,{scale:1,container:surface,connected:true,desktop_width:800,desktop_height:600,id_to_window:{1:{scale:1,update_offsets(){},screen_resized(){this.resizes=(this.resizes||0)+1}}},send:p=>packets.push(JSON.parse(JSON.stringify(p))),_get_monitors:()=>({}),position_float_menu(){}});
 context.client=client;
 client.floeDisplay=vm.runInContext('new FloeXpraDisplay(client)',context);client.floeDisplay.version=1;
 return {client,surface,browser,listeners,packets,shadow};
}

test('native density negotiates real pixels and DPI while pointer hits the same logical target',()=>{
 const {client:c,surface,packets}=setup();
 assert(c.set_display_density('native'));
 ...
 c.set_display_density('logical');assert.equal(c.getMouse({clientX:123,clientY:234,button:0}).x,123);
...
test('density rounds up to preserve toolkit geometry and is bounded',()=>{
 for(const [dpr,want] of [[0.8,1],[1,1],[1.25,2],[1.5,2],[2,2],[3,3],[8,4],[NaN,1]]){
  const {client:c}=setup(dpr);c.set_display_density('native');assert.equal(c.scale,want);
 }
});

test('monitor changes reconfigure density once and disconnect revokes listeners',()=>{
 const s=setup();s.client.set_display_density('native');const n=s.packets.length;
...
test('server display bounds cap backing density and are reconsidered on resize',()=>{
 ...
 c.floeDisplay.accept({'floe-display':1,max_desktop_size:[1920,1440]});
 c.set_display_density('native');assert.equal(c.scale,2);
 c.floeDisplay.accept({'floe-display':1,max_desktop_size:[3840,2160]});
 c._screen_resized();assert.equal(c.scale,3);
 c.floeDisplay.accept({'floe-display':1,max_desktop_size:[0,NaN]});
 assert.equal(c.floeDisplay.maximum,null);
});

test('shadow pointer keeps logical size and hotspot inside the scaled surface',()=>{
 const {client:c,shadow}=setup();c.set_display_density('native');
